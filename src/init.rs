...
pub fn init_page(root: &Path, file: &str, config: &Config) -> Result<()> {
    let target_root = root.join(&config.target_dir);
...
fn ensure_agents_md(fragments_dir: &Path, config: &Config) -> Result<()> {
    ...
    let prefix = &config.marker_prefix;
    let dir = &config.fragments_dir;

    let content = format!(
        r#"# Shared Fragments

Files in this folder are synced into target files by `fragments`.

## How it works

- Each `<name>.html` here is a shared fragment
- Pages opt in with marker pairs: `<!-- {prefix}:<name> -->...<!-- /{prefix}:<name> -->`
- `fragments sync` propagates changes from here into all pages
- `fragments check` verifies all pages are up to date (CI/pre-commit)

## Editing

1. Edit a fragment file in `{dir}/`
2. Run `fragments sync`
